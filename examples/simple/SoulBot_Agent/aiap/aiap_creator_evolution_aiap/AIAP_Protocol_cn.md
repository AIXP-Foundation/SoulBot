...
| `thinking_mode` | **思考模式** | 定义 L0 思考层和 L1 输出层的语言/编码（可选字段） |
...
### 2.4 thinking_mode 思考模式规则 [实验性]

规则：`thinking_mode` 定义 Agent 内部推理格式（L0）和面向用户的输出格式（L1）。

**字段结构**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `L0` | string | 是 | 思考层语言/编码格式 |
...
| 模式 | 值 | 说明 |
|------|------|------|
| hex（默认） | `"Thinking in hexadecimal machina code"` | AI 原生压缩推理模式 |
| English | `"Thinking in English"` | 完整推理深度 |
| native | `"Thinking in User's language"` | 用户语言推理 |

**默认行为**：当 `thinking_mode` 缺失时，使用运行时 Agent 的默认思考模式。这保证了与现有 AISOP 文件的向后兼容。

**安全覆盖规则**：模块 `id` 包含 "safety" 或在 system content 中标记 `security: true` 的模块，其 `thinking_mode.output` 必须锁定为 `"L1"`。全局或父级的 `output: "L0+L1"` 设置不传播到安全模块。这防止安全检测逻辑暴露给用户。
...
规则：
- 入口点：始终是 `RUN aisop.main`（instruction 不可变常量）
