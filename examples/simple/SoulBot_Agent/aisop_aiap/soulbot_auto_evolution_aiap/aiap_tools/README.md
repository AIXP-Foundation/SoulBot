...
This directory contains nested AIAP programs used as tools by `soulbot_auto_evolution`. Programs here are invoked via **intent execution** — the runtime reads their AISOP files and executes them node-by-node, not as API calls.

## Contents

| Directory | Program | Version | Purpose |
|-----------|---------|---------|---------|
| `soulbot_creator_evolution_aiap/` | AIAP Creator | v1.24.0 | Single-round AIAP evolution engine (full 15-stage pipeline) |

## Interface

### soulbot_creator_evolution_aiap
...
- When upgrading Creator, replace the entire `soulbot_creator_evolution_aiap/` directory with the new version.
