...
**基于 AISOP/AISIP 双协议和 AIAP 包系统的 AI Agent 框架**
...
SoulBot 是一个基于 Python 的 AI Agent 框架，通过 CLI 子进程（ACP 协议）连接 LLM —— **无需 API Key**。它引入了独特的架构：Agent 行为由 **AISOP 蓝图**（`.aisop.json`，Mermaid 流程图）或 **AISIP 蓝图**（`.aisip.json`，JSON 流程图）定义，通过 **AIAP 包**（`*_aiap`）扩展能力，使 AI Agent 的行为变得确定性、可复现、可版本控制。

### 核心特性

- **免 API Key** — 通过 Claude Code / Gemini CLI / OpenCode CLI 子进程接入 LLM
- **多模型切换** — Claude、Gemini、OpenCode（Kimi 等），`.env` 一行切换
- **AISOP/AISIP 双协议** — Agent 行为由 `.aisop.json`（Mermaid 流程图）或 `.aisip.json`（JSON 流程图）蓝图定义，自动检测协议格式
- **AIAP 包系统** — 热插拔功能包（`*_aiap`），支持 AISOP 和 AISIP 双格式入口，即插即用扩展 Agent 能力
...
| **AISIP V1.0.0** | AI Standard Interaction Procedure — AISOP 的姊妹协议，通过 **JSON 流程图**定义控制流，与 AISOP 共享 §0/§2/§5/§6 规范，仅 §4 流图格式不同 |
| **AIAP 包** | AI Application Package — 热插拔功能模块（`*_aiap/`），支持 `main.aisop.json` 或 `main.aisip.json` 双格式入口 |

**核心理念**：流程图是确定性执行路径（类似电路图），prompt 提供上下文和约束（类似元器件参数）。AISOP 用 Mermaid 语法（适合可视化），AISIP 用 JSON 语法（适合程序化处理）。这种分离使 Agent 行为可复现、可版本控制。
...
agent.py → _dynamic_instruction()
    ...
    ├── main.aisop.json 或 main.aisip.json（WHAT — 路由规则，自动检测协议）
    └── [Available AIAP packages]（可用能力，扫描 aisop_aiap/ 和 aisip_aiap/）
    ↓
LLM 按流程图执行：
    NLU[匹配意图] → Run[加载并执行 *_aiap/main.aisop.json 或 main.aisip.json]
    ↓
AIAP 包内的流程执行领域逻辑
    ↓
返回结果给用户
...
├── AISOP/AISIP 引擎 →  .aisop.json (Mermaid) / .aisip.json (JSON) → AIAP 包路由
...
完美运行 AISOP/AISIP 流程的大语言模型推荐：

| 模型 | 说明 |
|------|------|
| `gemini-3-flash-preview` | 快速、精准，推荐日常使用 |
| `gemini-3.1-pro-preview` | 更强推理能力，适合复杂流程 |

**当前最佳运行组合**：`SoulBot` + `Gemini CLI` + `gemini-3-flash-preview`
...
- AISOP/AISIP payload：定时任务携带完整 AISOP/AISIP V1.0.0 蓝图
- 持久化恢复：重启后自动恢复活跃任务
